...
void RB_DrawSun( float scale, shader_t *shader ) {
	...
	qglDepthRange ? qglDepthRange( 1.0, 1.0 ) : qglDepthRangef( 1.0, 1.0 );
	...
	qglDepthRange ? qglDepthRange( 0.0, 1.0 ) : qglDepthRangef( 0.0, 1.0 );
}
...
void RB_StageIteratorSky( void ) {
	...
	if ( r_showsky->integer ) {
		qglDepthRange ? qglDepthRange( 0.0, 0.0 ) : qglDepthRangef( 0.0, 0.0 );
	} else {
		qglDepthRange ? qglDepthRange( 1.0, 1.0 ) : qglDepthRangef( 1.0, 1.0 );
	}
